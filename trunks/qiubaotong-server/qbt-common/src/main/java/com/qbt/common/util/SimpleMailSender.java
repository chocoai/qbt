	...
	public boolean sendTextMail(MailSenderInfo mailInfo) {
		...
		try {
		   ...
		   InternetAddress[] sendTo = new InternetAddress[to.length];  
			...
			if(!StringUtil.isEmpty(mailInfo.getToCcAddress())){
				String[] tocc = mailInfo.getToCcAddress().split(";");
			    InternetAddress[] sendTocc = new InternetAddress[tocc.length];  
		        for (int i = 0; i < tocc.length; i++) {  
		        	sendTocc[i] = new InternetAddress(tocc[i]);  
		        }
				mailMessage.setRecipients(Message.RecipientType.CC, sendTocc);
			}
			// 设置邮件消息的主题
