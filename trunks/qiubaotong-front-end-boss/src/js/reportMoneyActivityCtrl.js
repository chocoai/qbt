...
.controller('reportMoneyActivityCtrl',['$scope','CommonService',function($scope,CommonService){
    ...
    $scope.getList = function(num){
        CommonService.pagination({
        	url : '/activity/activityFindByPage.api?token=' + localStorage.getItem('token'),
        	baseUrl:rBasetUrl,
            data :{
        		pageNo : num || "1",
    ...
    $scope.reportDownload = function(){
    	if(!$scope.startTime || !$scope.endTime){
	    	...
	    	window.open(rBasetUrl + '/activity/exportActivity.api?token=' + localStorage.getItem('token') + '&startDate=' + $scope.startTime1 +
	    	 '&endDate=' + $scope.endTime1 + '&name=' + $scope.activityName);
   		}
    }
