...
.controller('airportCheckDetailCtrl',['$scope','$http','CommonService',function($scope,$http,CommonService){
    ...
    $scope.addAirport = function(){
        ...
        if(!reg.test($scope.firstLetter)||$scope.firstLetter.length>50){
          showWarn("机场首字母只能输入英文字母");
          return false;
        }
        if(!$scope.address){
            showWarn("详细地址不能为空");
        ...
        if(!id){
             CommonService.httpRequest({
                ...
                showInfo("新增成功",function(){
                    tabOpenParent({name:'airportCheck',url:'airportCheck.html',displayName:'机场网点'});
                });
            });
        }
        else{
            CommonService.httpRequest({
                url : '/airport/update.api',
                method : 'post',
                data :{
                    id:$scope.id,
                    name : $scope.name,
