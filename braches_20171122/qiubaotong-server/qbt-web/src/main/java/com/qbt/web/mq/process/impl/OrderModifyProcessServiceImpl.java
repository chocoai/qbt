...
@Service
public class OrderModifyProcessServiceImpl extends AbstractProcessService{
	...
	public void processMsg(String msg) {
		try {
			...
			LogCvt.info("BPS逆向重新下单或纸质下单处理ID:"+msg);
			...
			if(orderInfo.getHandleType() == 2){
				OrderReverseReq orderReq = contactOrderReverseReq(orderInfo, kfUser);
				// 发送至顺丰
				...
				OrderReq orderReq = contactOrderReq(orderInfo, kfUser);
				// 发送至顺丰
				OrderResp resp = sfExpressApiService.orderService(orderReq);
				if(resp == null){
					LogCvt.error("订单支付成功，顺丰下单失败，msg:"+ "下发BSP, 接口无响应");
					orderInfo.setBpsStatus(SfBpsStatus.fail.getCode());
					orderInfoService.updateById(orderInfo);
					
					String desc = "订单支付成功，下发顺丰失败，原因："+"下发BSP, 接口无响应";
					OrderExceptionBook orderExceptionBook = saveOrderExceptionBook(orderInfo, desc);
	...
	private OrderReverseReq contactOrderReverseReq(OrderInfo orderInfo, UserSfKf kfUser) {
...
//	    orderAddedService.setValue(orderInfo.getInsuredValue() == 0 ? "2000" : String.valueOf(Arith.mul(orderInfo.getInsuredValue(),10000)));
	 ...
	 *  正向下单
	  ...
	  * @return
	 */
	private OrderReq contactOrderReq(OrderInfo orderInfo, UserSfKf kfUser) {
		...
		order.setJ_address(orderInfo.getjTreePath().replace(",", "")+orderInfo.getjAddress());
		// 收件方信息
		String[] dtreePaths = orderInfo.getdTreePath().split(",");
		String d_province = dtreePaths[0];
		String d_city = dtreePaths[1];
		...
		order.setD_address(orderInfo.getdTreePath().replace(",", "")+orderInfo.getdAddress());
		// other
		order.setPay_method("3");
		order.setExpress_type(orderInfo.getExpressType());
		order.setCargo_total_weight("18.00");
		order.setSendstarttime(DateUtil.formatDate(orderInfo.getSendSfUserTime(), DateUtil.DATE_TIME_FORMAT_01) );
		order.setRemark("节日派送,重件高价值物品请三轮车或面包车取包，需包装，取包:"+DateUtil.formatDate(orderInfo.getSendSfUserTime(),"MM-dd HH:mm:ss"));
