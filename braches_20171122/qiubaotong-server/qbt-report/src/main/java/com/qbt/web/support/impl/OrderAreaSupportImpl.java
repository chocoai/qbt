...
@Service
public class OrderAreaSupportImpl implements OrderAreaSupport {
	...
	public Map<String, Object> export(SysUser sysUser, String title, OrderAreaRequest req) {
		...
		do {
			...
			List<ReportSiteOrder> list = reportOrderService.findAreaOrders(pageEntity);
	...
	public List<OrderAreaResponse> findByPage(OrderAreaRequest req) {
		...
		List<ReportSiteOrder> list = reportOrderService.findAreaOrders(pageEntity);
		List<OrderAreaResponse> respList = BeanUtil.list2list(list, OrderAreaResponse.class);
