...
var agent_pay_view = Vue.extend({
    ...
    pay : function(){
      ...
      if(self.order_detail.amount == 0){
        $.commonAjax({
          ...
          success : function(data){
            self.paylock = false;
            self.goV2Nav('agent_v3.html','/agent_success/'+self.order_detail.sendTimeSection+'/'+self.order_detail.orderId);
          },
          error : function(err){
            self.paylock = false;
            $.toast(err);
          ...
          success : function(data){
            wx.chooseWXPay({
              ...
              success: function (res) {
                var hach='/agent_success/'+self.order_detail.sendTimeSection+'/'+self.order_detail.orderId;
                self.goV2Nav('agent_v3.html',hach);
              },
              fail: function(err){
  ...
  activated : function(){
    ...
    this.getDetail();
    //设置路由标示
    setStore('route_ident',"#agent_pay");
  },
  mounted : function(){
