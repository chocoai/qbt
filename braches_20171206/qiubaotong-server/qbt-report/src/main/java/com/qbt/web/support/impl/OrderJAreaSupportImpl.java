...
@Service
public class OrderJAreaSupportImpl implements OrderJAreaSupport {
	...
	public Map<String, Object> export(SysUser sysUser, String title, OrderJAreaRequest req) {
		...
		do {
			...
			List<ReportSiteOrder> list = reportOrderService.findJAreaOrders(pageEntity);
			if(Checker.isNotEmpty(list)){
				...
				for(ReportSiteOrder o : list){
					...
					data.put("jTreePath", o.getjTreePath());//寄件球场名称
					data.put("dTreePath", o.getdTreePath());//收件球场名称
	...
	public List<OrderJAreaResponse> findByPage(OrderJAreaRequest req) {
		...
		List<ReportSiteOrder> list = reportOrderService.findJAreaOrders(pageEntity);
		List<OrderJAreaResponse> respList = BeanUtil.list2list(list, OrderJAreaResponse.class);
