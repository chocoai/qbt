...
//					}
	 ...
	 * @param response
	 */
	@RequestMapping(value = "/ticketPacket/wechatpay")
	public void weixinPayTicketPackageReturn(HttpServletRequest request,HttpServletResponse response){
		boolean falg = false;
		try{
			String postStr = this.readStreamParameter(request.getInputStream());
			if (null != postStr) {
				...
				if("SUCCESS".equals(returnCode)){
					...
					//根据uuid获取原预支付信息流水
					TicketPackagePayment ticketPackagePayment = ticketPackagePyamentSupport.findByUuid(reqData.get("out_trade_no").toString());
					if(ticketPackagePayment == null){
						LogCvt.info("支付流水不存在，无法更新支付状态");
						return;
					}
					String transaction_id = reqData.get("transaction_id").toString();
					if(transaction_id.equals(ticketPackagePayment.getPayBakNo())){
						LogCvt.info("微信重复通知支付成功，此调微信异步信息不与处理："+transaction_id);
						falg = true;
						return;
					}
					...
					//更改券包的支付状态和激活券包
					ticketPackagePyamentSupport.createTicketPackageAfterPaid(ticketPackagePayment.getOrderId());
					
					ticketPackagePayment.setPayBakNo(transaction_id);
					
					//修改预支付订单状态
					ticketPackagePayment.setPayBakNo(transaction_id);
					ticketPackagePayment.setUpdateTime(new Date());
					ticketPackagePayment.setStatus(PaymentStatus.success.getCode());
					ticketPackagePayment.setPayStatus(PayStatus.success.getCode());
					//更新预支付订单信息，并推送微信通知
					ticketPackagePyamentSupport.updatePayment(ticketPackagePayment);
