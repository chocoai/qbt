...
.controller('reportToCourseCtrl',['$scope','CommonService',function($scope,CommonService){
    ...
    $scope.getList = function(num){
        ...
        CommonService.pagination({
        	url : '/orderCourse/findByPage.api?token=' + localStorage.getItem('token'),
            baseUrl : rBasetUrl,
        	data :{
        		pageNo : num || "1",
    ...
    $scope.reportDownload = function(){
    	if(!$scope.startTime || !$scope.endTime){
	    	...
	    	window.open(rBasetUrl + '/orderCourse/export.api?token=' + localStorage.getItem('token') 
