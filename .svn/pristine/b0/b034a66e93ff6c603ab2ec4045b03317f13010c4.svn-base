...
public class OrderSupportImpl implements OrderSupport {
	...
	public void update(OrderDetailVo orderDetail, SysUser sysUser) {
		...
		// 需要判断当前的订单类型是否是加急的，要更改为快件的express_type类型
		// 后台修改加急，实际顺丰的express_type还是快件1
		if("3".equals(newOrderInfo.getExpressType())) {
			newOrderInfo.setExpressType("1");
		}
...
//		}
	...
	public void createOrder(OrderConfirm orderConfirm) {
		...
		LogCvt.info("orderConfirm.getPrice(): "+orderConfirm.getPrice());
		...
		LogCvt.info("orderConfirm.getTotalPrice(): "+orderConfirm.getTotalPrice());
