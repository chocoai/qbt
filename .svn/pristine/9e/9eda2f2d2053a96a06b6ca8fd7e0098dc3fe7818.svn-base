...
.controller('reportFirstOrderCtrl',['$scope','CommonService',function($scope,CommonService){
    ...
    $scope.getList = function(num){
        CommonService.pagination({
        	url : '/orderFirst/findByPage.api?token=' + localStorage.getItem('token'),
        	baseUrl:rBasetUrl,
            data :{
        		pageNo : num || "1",
    ...
    $scope.reportDownload = function(){
    	if(!$scope.startTime || !$scope.endTime){
	    	...
	    	window.open(rBasetUrl + '/orderFirst/export.api?token=' + localStorage.getItem('token') + '&startDate=' + $scope.startTime1 +
