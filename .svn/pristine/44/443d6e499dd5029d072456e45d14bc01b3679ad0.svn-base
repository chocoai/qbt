...
public class CommonServiceImpl implements CommonService {
	...
	public String createTicketPackagePaymentId() {
		String yyymmdd = DateUtil.formatDate(new Date(), DateUtil.DATE_FORMAT_YYMMDD);
		String key = RedisKeyUtil.serial_number_increment_key(SerialTypeEnum.TICKET_PACKAGE_PAYMENT_TYPE.getType(), yyymmdd);
		long exists = redisService.setnx(key, "1");
		long number = 1;
		if(exists == 1){
			redisService.setex(key, 25 * 60 * 60, "1");
		...
		return ToolsUtil.createNo("",yyymmdd, SerialTypeEnum.TICKET_PACKAGE_PAYMENT_TYPE.getValue(), number);
	}
