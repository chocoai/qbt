var order_list_view = Vue.extend({
        ...
        methods: {
            ...
            cancel: function (item, type) {
                ...
                var str;
                //if(isCurrent==true){
                //Vincent 2017-11-23 update 待支付状态是不需要扣除手续费的
                if (isCurrent == true && self.orderStatus != 1) {
                ...
                $.confirm(str, function () {
                    ...
                    if (self.orderStatus == 1) {
                        ...
                        $.commonAjax({
                            ...
                            type: 'post',
                            data: {orderId: item.number},
                            //Vincent Chang 2017-11-23 update
                            //后台api是根据order_info的number字段去查找的,当待支付时是subNumber、其他状态是number
                            //data: {orderId: self.orderStatus == 1 ? item.subNumber : item.number},
                            success: function (data) {
                                $.alert('取消成功', function () {
            ...
            backOrder: function (item) {
                ...
                $.commonAjax({
                    ...
                    data: {
                        ...
                        //Vincent Chang 2017-11-23 update
                        //后台api是根据order_info的number字段去查找的,当待支付时是subNumber、其他状态是number
                        orderId: self.orderStatus == 1 ? item.subNumber : item.number
                    },
                    success: function (data) {
                        console.log(data);
                        var datas = data.datas;
