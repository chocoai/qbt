    ...
    .controller('orderDownCtrl', ['$scope', 'CommonService', 'Constants', '$timeout', function ($scope, CommonService, Constants, $timeout) {
            ...
            CommonService.pagination({
                ...
                if (!data.item[0].wxUserId) {
                    layer.confirm('需关联微信用户才能下单，是否前往平台用户管理页面进行关联？', {
                        ...
                        tabOpenParent({
                            name: 'manageCheck',
                            url: 'manageCheck.html?loginMobile=' + localStorage.getItem('loginMobile'),
                            displayName: '平台用户管理'
                        });
                    });
                }
        ...
        $scope.getBusinessType = function () {
            CommonService.httpRequest({
                ...
                if (urgentItem) {
                    tempList.push(urgentItem);
                }
                $scope.deliverTimes = tempList;
                //默认选中快件
                $('.checkDeliverTime').eq(0).prop('checked', true);
                $scope.order.deliverDate = $scope.deliverTimes[0].deliver_time.split(',')[1];
                $scope.order.expressType = $scope.deliverTimes[0].business_type;
                $scope.order.sfPrice = $scope.deliverTimes[0].sf_price;
                $scope.order.outRangePrice = $scope.deliverTimes[0].out_range_price;
                $scope.getFullCut();
            });
        };
        ...
        $scope.checkOrderIsVip = function () {
            CommonService.httpRequest({
                ...
                CommonService.httpRequest({
                    ...
                    if ($scope.order.isVipOrder && $('.checkDeliverTime').eq(0).prop('checked') == true) {
                    ...
                    } else if ($scope.order.isVipOrder && $('.checkDeliverTime').eq(1).prop('checked') == true) {
                        $scope.order.cutAmount = ($scope.order.quantity - $scope.order.vipNumber) * ($scope.order.fcDAmount + $scope.order.fcJAmount);
                        $scope.order.normalAllAmount = ($scope.order.quantity - $scope.order.vipNumber)
                            * ($scope.order.sfPrice + $scope.order.amountPrice - $scope.inputDiscount)
                            - $scope.order.cutAmount;
        ...
        $scope.showOrder = function () {
            if (!$scope.order.jAreaId || !$scope.order.dAreaId || !$scope.order.sendSfUserTime || !$scope.order.deliverDate || !$scope.order.sfPrice || !$scope.order.langName ||
            ...
            if ($scope.inputDiscountMsg) {
                showWarn($scope.inputDiscountMsg);
                return;
            }
