...
public class ExcelParseSfPrice {
	...
	private static void readXls() throws IOException{  
      ...
      for(int rowNum = 3; rowNum <= hssfSheet.getLastRowNum(); rowNum++){  
        	...
        	// 结束城市
        	boolean destCity = getBoolValue(hssfRow.getCell(6)); 
...
//        			+ "VALUES (now(), now(), '"+srcProv+"', '"+srcCity+"', '"+destProv+"','"+destCity+"', "+cichenSfPrice+", "+cichenQbtprice+","+cichenFprice+","+cichenXprice+","+ciriSfPrice+", "+ciriQbtprice+","+ciriFprice+","+ciriXprice+","+jriSfPrice+", "+jriQbtprice+","+jriFprice+","+jriXprice+","+geriSfPrice+", "+geriQbtprice+","+geriFprice+","+geriXprice+");";
        	...
        	String sql = "update `base_sf_price` set "
        	...
        	lines.add(sql);
        	if(rowNum % 5000 == 0){
        		FileUtils.writeLines(new File("/Volumes/work/new_base_sf_price_update.sql"), lines, true);
        		lines.clear();
        	}
      	}
	  	FileUtils.writeLines(new File("/Volumes/work/new_base_sf_price.sql"), lines, true);
		lines.clear();
	}
